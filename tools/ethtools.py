...
def mint_coin(address, amount, token_id = "erc1155"):
...
def get_token_ids(start = 0, limit = 10, token_id = TOKEN_DEF_1155):
...
def get_token_id_total_amount(id, token_id = "erc1155"):
...
def mint_brand(manager, to, brand, data = None, token_id = "erc1155"):
    wallet = get_ethwallet()
    ret = wallet.get_account(manager)
    if ret.state != error.SUCCEED:
        raise Exception("get account failed")
...
def mint_type(manager, token_id, to, brand, btype, data = None, token_id = "erc1155"):
    wallet = get_ethwallet()
    ret = wallet.get_account(manager)
    if ret.state != error.SUCCEED:
        raise Exception("get account failed")
